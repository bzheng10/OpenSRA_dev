...
def main(work_dir, logging_level='info', logging_message_detail='simple',
    ...
    if 'site_data_PROCESSED_CROSSING_ONLY.csv' in os.listdir(processed_input_dir):
        ...
        if prob_crossing[0] == 1:
            ...
            # segment_ids_crossed = list(site_data.ID.values) # convert to list to use count
            # segment_ids_crossed_unique = np.unique(segment_ids_crossed)
            # segment_ids_crossed_repeat = np.asarray([
            #     val for val in segment_ids_crossed_unique
            #     if segment_ids_crossed.count(val)>1
            # ])
        else:
            possible_repeated_crossings = False
            # segment_ids_full = site_data_full.ID.values
            # segment_ids_crossed = list(site_data.ID.values)
            # segment_ids_crossed_unique = np.unique(segment_ids_crossed)
            # # segment_ids_crossed_repeat = np.asarray([])
            # segment_ids_crossed_repeat = np.asarray([
            #     val for val in segment_ids_crossed_unique
        ...
        if 'caprock_leakage' in workflow_i['haz_list']:
            ...
            # for event_ind in range(8,9):
                ...
                if running_cpt_based_procedure is False:
                    ...
                    # pga_curr_event = im_import['pga']['mean_table'][event_ind,:].copy()
                    pga_curr_event = np.round(im_import['pga']['mean_table'][event_ind,:].copy()-10,decimals=3)
                    
                    # get sites with nonzero IMs
                    # sites_with_zero_pga = np.where(im_dist_info['pga']['mean']>0)[0]
                    # sites_with_nonzero_step0 = np.asarray(list(set(n_site_ind_arr.copy()).difference(set(sites_with_zero_pga))))
                    # sites_with_nonzero_step0 = np.where(pga_curr_event>0)[0]
                    ...
                    # print('\n\n', event_ind, '___________')
                    # print('number of nonzero sites for current event', n_site_curr_event)
                    ...
                    # print(im_dist_info['pga']['mean'][:10])
                    # print(min(im_dist_info['pga']['mean']))
                    # sys.exit()
                    
                    # sys.exit()
                # get subset of site_data using sites with nonzero PGA
                # site_data_for_curr_event = site_data.loc[sites_with_nonzero_step0].copy()
                # nonzero_pga_site_arr = 
                ...
                if infra_type == 'wells_caprocks':
                    ...
                    if sites_with_no_crossing is None:
                        for key in list(rup_info):
                            if key == 'theta':
                                # sites_with_no_crossing = list(set(n_site_ind_arr.copy()).difference(set(sites_with_crossing)))
                ...
                if flag_crossing_file:
                    ...
                    addl_input_dist = {}
                    # null_arr_nsite = np.zeros(n_site)
                    # ones_arr_nsite_by_ninput = np.ones(n_site)
                        ...
                        for param in crossing_params:
                            ...
                            if param_dist_type == 'fixed':
                                ...
                                if param_mean[0] == 'event_dependent':
                                    # sigma
                                    # sigmas = ones_arr_nsite_by_ninput.copy()
                ...
                # input_samples_nsite_nonzero = {}
                # for param in input_samples:
                #     input_samples_nsite_nonzero[param] = input_samples[param][sites_with_nonzero_step0].copy()
                if get_timer:
                ...
                # print(len(sites_with_nonzero_step0))
                # for param in input_samples_nsite_nonzero:
                #     print(param, input_samples_nsite_nonzero[param].shape)
                # sys.exit()
                ...
                if running_cpt_based_procedure:
                    ...
                    # if step0_str in prev_case_params and \
                    ...
                    #     mean_of_mu[step0_str] = prev_mean_of_mu[step0_str].copy()
                    #     sigma_of_mu[step0_str] = prev_sigma_of_mu[step0_str].copy()
                    #     sigma[step0_str] = prev_sigma[step0_str].copy()
                    ...
                    # print(mean_of_mu[step0_str]['pga'])
                    # print(np.min(mean_of_mu[step0_str]['pga']))
                    # sys.exit()
                ...
                for step in range(1,pbee_dim-1):
                    ...
                    # if curr_step_str in prev_case_params and \
                    #     prev_case_params[curr_step_str]['cat'] == curr_cat and \
                    #     prev_case_params[curr_step_str]['haz'] == curr_haz and \
                    #     prev_case_params[curr_step_str]['methods'] == list(curr_methods):
                        
                    #     print(f'skipping step {step}')
                        
                    #     # use results from analysis of previous case
                    #     mean_of_mu[curr_step_str] = prev_mean_of_mu[curr_step_str].copy()
                    #     sigma_of_mu[curr_step_str] = prev_sigma_of_mu[curr_step_str].copy()
                    #     sigma[curr_step_str] = prev_sigma[curr_step_str].copy()
                    #     amu[curr_step_str] = prev_amu[curr_step_str].copy()
                    #     bmu[curr_step_str] = prev_bmu[curr_step_str].copy()
                    ...
                    # else:
                        # n_site_to_use = n_site_curr_event
                    ...
                    # if running_cpt_based_procedure is False:
                            ...
                            if 'liq_susc' in input_dist:
                                ...
                                if not 'liq_susc' in input_samples:
                                    ...
                                    liq_susc_forward = np.tile(
                                        # input_dist['liq_susc']['value'][sites_with_nonzero_step0],
                        ...
                        #
                        # print(curr_step_str, has_nonzero_mean)
                        # if has_nonzero_mean:
                        rows_inputs = rows_to_keep_rel_to_nonzero_step0[prev_step_str].copy()
                        # print('getting inputs:', rows_to_keep_rel_to_nonzero_step0[prev_step_str])
                        ...
                        # print('input samples')
                        # for each in curr_input_samples:
                        #     print(curr_input_samples[each].shape)
                        
                        
                        # print('upstream samples')
                        # for each in curr_upstream_params:
                        #     print(curr_upstream_params[each].shape)
                            
                            
                        # print('internal samples')
                        # for each in curr_internal_params:
                        #     print(curr_internal_params[each].shape)
                        ...
                        for param in curr_results:
                            if param in min_mean_for_zero_val[curr_cat]:
                                ...
                                # print(list(curr_results[param]))
                                # print(curr_results[param]['mean_of_mu'].shape)
                                # print(max(curr_results[param]['mean_of_mu']))
                                # print(min(curr_results[param]['mean_of_mu']))
                                # print(threshold_val)
                                # print(rows_nonzero_mean_for_curr_param)
                                # print(curr_results[param])
                        ...
                        # print('\n',curr_step_str,'>>>>>>>>>>>>>>>')
                        # print(rows_to_keep_index[curr_step_str])
                        # print(rows_to_keep_rel_to_nonzero_step0[curr_step_str])
                        # print(sites_to_keep[curr_step_str])
                        # print(curr_step_str,'>>>>>>>>>>>>>>>')
                        ...
                        if len(nonzero_ind_from_out) == 0:
                            ...
                            # print(f'--------breaking out from {curr_step_str}')
                            break
                    ...
                    # based on sites with nonzero means, determine which rows to keep
                    # rows_to_keep = []
                    # print(sites_with_nonzero_step0)
                    # for param in rows_to_keep[curr_step_str]:
                    #     print(param)
                    #     print(rows_to_keep[curr_step_str][param][0])
                    #     print(np.where(sites_with_nonzero_step0==rows_to_keep[curr_step_str][param][0]))
                    #     if len(rows_to_keep[curr_step_str][param]) > 0:
                    #         rows_to_keep += [
                    #             np.where(sites_with_nonzero_step0==idx)[0]
                    #             for idx in rows_to_keep[curr_step_str][param]
                    #         ]
                    ...
                    for i,param in enumerate(curr_haz_param):
                        ...
                        curr_bmu[param] = \
                            curr_slope[param]*(-mean_of_mu[prev_step_str][prev_param_to_use][rows_to_keep_index[curr_step_str]]) + curr_intercept[param]
                ...
                if has_nonzero_mean:
                    ...
                    # # if current step setup params are the same as the previous case, then skip analysis
                    # if last_step_str in prev_case_params and \
                    #     prev_case_params[last_step_str]['cat'] == last_cat and \
                    #     prev_case_params[last_step_str]['haz'] == last_haz and \
                    #     prev_case_params[last_step_str]['methods'] == list(last_methods):
                        
                    #     print(f'skipping step {last_step}')
                        
                    #     # use results from analysis of previous case
                    #     mean_of_mu[last_step_str] = prev_mean_of_mu[last_step_str].copy()
                    #     sigma_of_mu[last_step_str] = prev_sigma_of_mu[last_step_str].copy()
                    #     sigma[last_step_str] = prev_sigma[last_step_str].copy()
                ...
                if has_nonzero_mean is False:
                    ...
                    for i, param_i in enumerate(last_haz_param):
                        ...
                        if not param_i in pc_coeffs:
                            pc_coeffs[param_i] = null_arr_pc_terms.copy()
                        else:
                    ...
                    # print('\n')
                    # for step in mean_of_mu:
                    #     print(step)
                    #     for param in mean_of_mu[step]:
                    #         print(param)
                    #         print(mean_of_mu[step][param])
                    
                    
                    # print('\n')
                    # for step in rows_to_keep_index:
                    #     print(step)
                    #     print(rows_to_keep_index[step])
                    #     print(rows_to_keep_rel_to_nonzero_step0[step])
                    #     print(sites_to_keep[step])
                    ...
                    # print(mean_of_mu['step_0'])
                    # print('\n')
                    # print(mean_of_mu['step_1'])
                    # print('\n')
                    # print(mean_of_mu['step_2'])
                    # print('\n')
                    
                    # print(input_samples['beta_crossing'])
                    # print(mean_of_mu['step_2']['eps_pipe_comp'])
                    # print(input_samples['beta_crossing'][np.isnan(mean_of_mu['step_2']['eps_pipe_comp'])])
                    # sys.exit()
                    
                    # initial step string
                    init_step_str = 'step_0'
                    
                    # index to use, based on nonzero sites in last step
                    # rows_to_keep_rel_to_nonzero_step0[last_step_str]
                    ...
                    for i, param_i in enumerate(last_haz_param):
                        ...
                        for step in range(1, pbee_dim):
                            if step == 1:
                                ...
                                pc_kwargs[f'mu{pc_letter_for_step[0]}'] = mean_of_mu[init_step_str][param_to_use][rows_to_use_pc[init_step_str]]
                                pc_kwargs[f'sigmaMu{pc_letter_for_step[0]}'] = sigma_of_mu[init_step_str][param_to_use][rows_to_use_pc[init_step_str]]
                                pc_kwargs[f'sigma{pc_letter_for_step[0]}'] = sigma[init_step_str][param_to_use][rows_to_use_pc[init_step_str]]
                            if step == pbee_dim-1:
                                pc_kwargs[f'mu{pc_letter_for_step[step]}'] = mean_of_mu[last_step_str][param_i][rows_to_use_pc[last_step_str]]
                                pc_kwargs[f'sigmaMu{pc_letter_for_step[step]}'] = sigma_of_mu[last_step_str][param_i][rows_to_use_pc[last_step_str]]
                                pc_kwargs[f'sigma{pc_letter_for_step[step]}'] = sigma[last_step_str][param_i][rows_to_use_pc[last_step_str]]
                            else:
                                step_str = f'step_{step}'
                                ...
                                if infra_type == 'below_ground' and step == pbee_dim-2:
                                    ...
                                    if len(params_for_step) == len(curr_haz_param) and len(curr_haz_param) > 1:
                                        ...
                                        param_to_use = params_for_step[0]
                                pc_kwargs[f'amu{pc_letter_for_step[step]}'] = amu[step_str][param_to_use][rows_to_use_pc[step_str]]
                                pc_kwargs[f'bmu{pc_letter_for_step[step]}'] = bmu[step_str][param_to_use][rows_to_use_pc[step_str]]
                                pc_kwargs[f'sigmaMu{pc_letter_for_step[step]}'] = sigma_of_mu[step_str][param_to_use][rows_to_use_pc[step_str]]
                                pc_kwargs[f'sigma{pc_letter_for_step[step]}'] = sigma[step_str][param_to_use][rows_to_use_pc[step_str]]
                        ...
                        # print(pc_coeffs_param_i.shape)
                        pc_coeffs_param_i = pc_coeffs_param_i * rup_info['rate']
                        
                        # print(pc_coeffs_param_i.shape)
                        # print(sites_with_nonzero_step0.shape)
                        ...
                        if not param_i in pc_coeffs:
                            ...
                            pc_coeffs[param_i] = pc_coeffs_param_i_full
                        else:
                            # pc_coeffs[param_i] = pc_coeffs[param_i] + pc_coeffs_param_i * rup_info['rate'] # weighted by annual rate
                            pc_coeffs[param_i] = pc_coeffs[param_i] + pc_coeffs_param_i_full
            ...
            time_loop = time.time()
                ...
                # # keep a copy of the mean, sigma, sigma_mu to not repeat analysis again
                # prev_mean_of_mu = mean_of_mu.copy()
                # prev_sigma_of_mu = sigma_of_mu.copy()
            ...
            for i,param_i in enumerate(last_haz_param):
                ...
                # print(second_to_last_haz_param)
                # print(param_i)
                # print(param_to_use)
                
                # round to N sig figs
                # df_frac_vals = df_frac[f'case_{case_to_run}'].values
                # nonzero_frac_loc = df_frac_vals>0
                # nonzero_frac = df_frac_vals[nonzero_frac_loc].values
                # print(nonzero_frac)
                # nonzero_frac_rounded = np.round(nonzero_frac,4-np.floor(np.log10(np.abs(nonzero_frac))).astype(int))
                # df_frac[f'case_{case_to_run}'][nonzero_frac_loc] = nonzero_frac_rounded
            ...
            if flag_crossing_file:
                if infra_type == 'below_ground':
                    if 'landslide' in workflow['EDP']:
                        ...
                        df_frac[f'case_{case_to_run}'] = df_frac[f'case_{case_to_run}'] * np.tile(prob_crossing,(6,1)).T
                        # return_frac_unique = pd.DataFrame(None,columns=return_frac.columns)
                        # get fractiles from crossed table, loop through via unique segment IDs
                        # print(segment_ids_crossed)
                        # print(df_frac[f'case_{case_to_run}'])
                        
                        # initialize fractile table with all locations
                        frac_full = pd.DataFrame(
                            0,
                            index=segment_index_full,
                            columns=df_frac[f'case_{case_to_run}'].columns
                        )
                        
                        # track_ind_for_segment_id = []
                        # print(segment_ids_full)
                        # if prob crossing == 1, then using deformation polygons and multiple crossings per segment is possible
                        # if prob crossing == 0.25, then no geometry was used and only 1 crossing per segment
                        
                        # for all of the unrepeated crossings, directly assign to frac_full
                        # frac_full.loc[segment_index_crossed_single] = 
                        
                        
                        # make a copy of the fractile table
                        # df_frac_single = df_frac[f'case_{case_to_run}'].copy()
                        
                        df_frac_index = df_frac[f'case_{case_to_run}'].index.values
                        
                        
                        # print('df_frac_index',df_frac_index)
                        # print(len(df_frac_index))
                        ...
                        for ind, segment_id in enumerate(segment_ids_crossed_repeat):
                            # print(segment_id)
                            # ind_in_full_for_segment_id = np.where(segment_ids_full==segment_id)[0][0]
                            ind_in_full_for_segment_id = segment_index_repeat_in_full[ind]
                            # track_ind_for_segment_id.append(ind_in_full_for_segment_id)
                            # print(segment_id, ind_in_full_for_segment_id)
                            # if segment_id in segment_ids_crossed_repeat:
                            # print(1)
                            # rows with repeating ID
                            rows = np.where(segment_ids_crossed==segment_id)[0]
                            # print('rows',rows)
                            df_frac_index = np.delete(df_frac_index,rows)
                            # print('df_frac_index',df_frac_index)
                            # print('ind_in_full_for_segment_id',ind_in_full_for_segment_id)
                            
                            
                            # df_frac_index.
                            
                            frac_repeat_curr_segment = df_frac[f'case_{case_to_run}'].loc[rows].reset_index(drop=True)
                            # pick case with higher mean value
                            worst_row = np.argmax(frac_repeat_curr_segment.iloc[:,-1])
                            # print(frac_repeat_curr_segment)
                            # print(worst_row)
                            # print(ind_in_full_for_segment_id)
                            # frac_to_store = 
                        ...
                        frac_full.loc[segment_index_single_in_full] = df_frac[f'case_{case_to_run}'].loc[df_frac_index].values
                        
                        # print(segment_index_crossed_single)
                        # sys.exit()
                        
                            # else:
                                # print(2)
                                # row = np.where(segment_ids_crossed==segment_id)[0][0]
                                # frac_to_store = df_frac[f'case_{case_to_run}'].loc[row].values
                                # print(frac_to_store)
                                # print('not unique', segment_id, row)
                                
                                
                            # frac_to_store.reset_index(drop=True,inplace=True)
                            # frac_to_store.to_csv(
                            #     rf'C:\Users\barry\OneDrive - SlateGeotech\CEC\ShakeMap\CJ_PreReroute_Level2\Results\{segment_id}.csv'
                            # )
                            # print(segment_id, frac_to_store)
                            # store to full fractile table
                            # frac_full.loc[ind_in_full_for_segment_id] = frac_to_store
                            # print(frac_full.loc[ind_in_full_for_segment_id])
                        # update df_frac
                        df_frac[f'case_{case_to_run}'] = frac_full.copy()
                        # set anything below 1e-10 to 0
                        df_frac[f'case_{case_to_run}'][df_frac[f'case_{case_to_run}']<1e-10] = 0
                        
                        # print(df_frac[f'case_{case_to_run}'].loc[track_ind_for_segment_id])
                        # return_frac_unique.
                        # sys.exit()
            ...
            # round to N sig figs
            # df_frac_vals = df_frac[f'case_{case_to_run}'].values
            # nonzero_frac_loc = np.where(df_frac[f'case_{case_to_run}']>0)
            # nonzero_frac = df_frac[f'case_{case_to_run}'].iloc[df_frac[f'case_{case_to_run}']>0].values
            # print(nonzero_frac)
            # nonzero_frac_rounded = np.round(nonzero_frac,4-np.floor(np.log10(np.abs(nonzero_frac))).astype(int))
            # df_frac[f'case_{case_to_run}'][nonzero_frac_loc] = nonzero_frac_rounded
            ...
            df_frac[f'case_{case_to_run}'].index = index
