...
class HutabaratEtal2022(SurfaceFaultRuptureInducedPipeStrain):
    ...
    def _model(
        ...
        #####
        # one of the 18 options:
        # 
        # beta_crossing <= 10
        #       1)                               normal
        # 10 < beta_crossing <= 90
        #       2)  -180 <= theta_slip <  -90    ss comp and normal
        #       3)   -90 <= theta_slip <=   0    ss tens and normal
        #       4)     0 <  theta_slip <=  90    ss tens and reverse
        #       5)    90 <  theta_slip <= 180    ss comp and reverse
        # 90 < beta_crossing < 170
        #       6)  -180 <= theta_slip <  -90    ss tens and normal
        #       7)   -90 <= theta_slip <=   0    ss comp and normal
        #       8)     0 <  theta_slip <=  90    ss comp and reverse
        #       9)    90 <  theta_slip <= 180    ss tens and reverse
        # beta_crossing >= 170
        #      10)                               reverse
        #####
        ...
        abs_cos_theta = np.abs(np.cos(np.radians(theta_slip)))
        abs_sin_theta = np.abs(np.sin(np.radians(theta_slip)))
        abs_beta_minus_180 = np.abs(beta_crossing-180)
        abs_cos_90_minus_beta = np.abs(np.cos(np.radians(90-beta_crossing)))
        pgdef_transverse = pgdef * abs_cos_theta
        pgdef_parallel = pgdef * abs_sin_theta / abs_cos_90_minus_beta
        
        ###################################################
        # for all cases where beta_crossing <= 10
        cond = beta_crossing <= 10
        if True in cond:
            case_to_run[cond] = 'Normal'
            ##################
            # Normal
            pgdef_to_use = pgdef[cond].copy()
        ...
        ###################################################
        # for all cases where 10 < beta_crossing <= 90
        cond1 = logical_and(beta_crossing > 10, beta_crossing <= 90)
        
        # 2)  -180 <= theta_slip <  -90    ss comp and normal
        cond2 = logical_and(theta_slip >= -180, theta_slip < -90)
        cond = logical_and(cond1, cond2)
        if True in cond:
            case_to_run[cond] = 'SSComp_and_Normal'
            ##################
            # SSComp
            beta_cond = abs_beta_minus_180[cond].copy()
            pgdef_to_use = pgdef_transverse[cond].copy()
            ...
            # Normal
            pgdef_to_use = pgdef_parallel[cond].copy()
        ...
        cond = logical_and(cond1, cond2)
        if True in cond:
            case_to_run[cond] = 'SSTens_and_Normal'
            ##################
            # SSTens
            beta_cond = beta_crossing[cond].copy()
            pgdef_to_use = pgdef_transverse[cond].copy()
            output1 = HutabaratEtal2022_SSTens._model(
                pgdef_to_use, # upstream PBEE RV
                d_pipe[cond], t_pipe[cond], sigma_y[cond], n_param[cond], r_param[cond], l_anchor[cond], # infrastructure
                beta_cond, h_pipe[cond], # geotech - general
                alpha_backfill[cond], s_u_backfill[cond], # clay
                gamma_backfill[cond], phi_backfill[cond], delta_backfill[cond], # sand
        ...
        cond = logical_and(cond1, cond2)
        if True in cond:
            case_to_run[cond] = 'SSTens_and_Reverse'
            ##################
            # SSTens
            beta_cond = beta_crossing[cond].copy()
            pgdef_to_use = pgdef_transverse[cond].copy()
        ...
        cond = logical_and(cond1, cond2)
        if True in cond:
            case_to_run[cond] = 'SSComp_and_Reverse'
            ##################
            # SSComp
            beta_cond = abs_beta_minus_180[cond].copy()
            pgdef_to_use = pgdef_transverse[cond].copy()
        ...
        ###################################################
        # for all cases where 90 < beta_crossing < 170
        cond1 = logical_and(beta_crossing > 90, beta_crossing < 170)
        ...
        ###################################################
        # for all cases where beta_crossing >= 170
        cond = beta_crossing >= 170
        if True in cond:
            ...
            ##################
            eps_pipe_comps[cond] = output['eps_pipe']['mean']
