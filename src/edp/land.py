...
def bray_macedo_2019(**kwargs):
    ...
    ## repeat 1d array to create matrix of shape = n_rup x n_site
    # ky = np.transpose(np.repeat(ky[:,np.newaxis],n_rup,axis=1))
    # M = np.repeat(M[:,np.newaxis],n_site,axis=1)
    # if slope_loc == 90:
        # slope_loc = np.ones([n_rup,n_site])*slope_loc
    # else:
        # slope_loc = np.transpose(np.repeat(slope_loc[:,np.newaxis],n_rup,axis=1))
    ...
    if 'ord' in gm_type.lower():
        ...
        a1 = np.zeros([n_rup,n_site])
        a2 = np.zeros([n_rup,n_site])
        a3 = np.zeros([n_rup,n_site])
    ...
    for k in range(n_samp_im):
        ...
        if 'ord' in gm_type.lower():
            ...
            if 'p_land' in return_param:
                p_D_eq_0_k = np.zeros([n_rup,n_site])
        ...
        d.update({k:sparse.coo_matrix((d_k_red,(row_k_red,col_k_red)),shape=(n_rup,n_site))})

        ## update dictionary if p_land is inquired
        if 'p_land' in return_param:
            p_land_k_red = p_land_k[ind_red]
            p_land.update({k:sparse.coo_matrix((p_land_k_red,(row_k_red,col_k_red)),shape=(n_rup,n_site))})
