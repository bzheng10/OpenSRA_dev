...
def zhu_etal_2017(**kwargs):
    ...
    if vs30 is None or precip is None or dc is None:
        ...
        if 'p_liq' in return_param or 'areal_liq' in return_param:
            ...
            liq_susc = np.transpose(np.repeat(liq_susc[:,np.newaxis],n_rup,axis=1))
            b_ln_pgv = np.transpose(np.repeat(b_ln_pgv[:,np.newaxis],n_rup,axis=1))
            vs30 = np.transpose(np.repeat(vs30[:,np.newaxis],n_rup,axis=1))
            dc = np.transpose(np.repeat(dc[:,np.newaxis],n_rup,axis=1))
...
def hazus_2014_liq(**kwargs):
    ...
    for k in range(n_samp_im):
        ...
        p_liq_pga = np.ones(pga_k.shape)*np.nan
        ...
        p_ml = np.ones(pga_k.shape)*np.nan
