...
def bray_macedo_2019(**kwargs):
	...
	dim_d = kwargs.get('dim_d',None) # number of sites
	dim_rup = kwargs.get('dim_rup',None) # number of ruptures
	nsamp = kwargs.get('nsamp',1) # number of samples, default to 1
	...
	## If user defines Ts <= 0.01 sec, then use pga, otherwise interpolate Sa(Ts_deg)
	# if Ts <= 0.01:
		# Ts_deg = None
		# Sa_Ts_deg = None
	# else:
	...
	## repeat 1d array to create matrix of shape = dim_rup x dim_d
	# ky = np.transpose(np.repeat(ky[:,np.newaxis],dim_rup,axis=1))
	# M = np.repeat(M[:,np.newaxis],dim_d,axis=1)
	# if slope_loc == 90:
		# slope_loc = np.ones([dim_rup,dim_d])*slope_loc
	# else:
		# slope_loc = np.transpose(np.repeat(slope_loc[:,np.newaxis],dim_rup,axis=1))
	...
	if 'ord' in gm_type.lower():
		...
		a1 = np.zeros([dim_rup,dim_d])
		a2 = np.zeros([dim_rup,dim_d])
		a3 = np.zeros([dim_rup,dim_d])
	...
	for k in range(nsamp):
		...
		if 'ord' in gm_type.lower():
			...
			if 'p_land' in return_param:
				p_D_eq_0_k = np.zeros([dim_rup,dim_d])
			...
			# ln_d_k = a1 - 2.491*np.log(ky) - 0.245*np.multiply(np.log(ky),np.log(ky)) +\
					# 0.344*np.multiply(np.log(ky),np.log(Sa_Ts_deg_k)) + 2.703*np.log(Sa_Ts_deg_k) -\
					# 0.089*np.multiply(np.log(Sa_Ts_deg_k),np.log(Sa_Ts_deg_k)) + a2*Ts +\
					# a3*np.multiply(Ts,Ts) + 0.607*M + a4*np.log(pgv_k) + a5
			...
			# ln_d_k = np.zeros([dim_rup,dim_d])

			## probably of displacement = 0 (threshold = 0.5 cm)
			if 'p_land' in return_param:
				p_D_eq_0_k = np.zeros([dim_rup,dim_d])
				p_D_eq_0_k[Ts<=0.7] = 1-norm.cdf(-2.46 - 2.98*np.log(ky[Ts<=0.7]) -\
										0.12*np.multiply(np.log(ky[Ts<=0.7]),np.log(ky[Ts<=0.7])) -\
										0.71*np.multiply(Ts[Ts<=0.7],np.log(ky[Ts<=0.7])) +\
										1.69*Ts[Ts<=0.7] + 2.76*np.log(Sa_Ts_deg_k[Ts<=0.7])) # eq. 8(a)
				p_D_eq_0_k[Ts>0.7] = 1-norm.cdf(-3.40 - 4.95*np.log(ky[Ts>0.7]) -\
										0.30*np.multiply(np.log(ky[Ts>0.7]),np.log(ky[Ts>0.7])) -\
										0.33*np.multiply(Ts[Ts>0.7],np.log(ky[Ts>0.7])) -\
										0.62*Ts[Ts>0.7] + 2.85*np.log(Sa_Ts_deg_k[Ts>0.7])) # eq. 8(b)
				p_land.update({k:sparse.coo_matrix(1-p_D_eq_0_k)})
		...
		d.update({k:sparse.coo_matrix((d_k_red,(row_k_red,col_k_red)),shape=(dim_rup,dim_d))})
		
		if k == 1:
			indx = ind_red[:3]
			print(indx,row_k_red[:3],col_k_red[:3],d_k_red[:3])
			print(a1[indx],a2[indx],a3[indx],a4[indx],a5[indx])
			print(M_k[indx],ky_k[indx],pgv_k[indx],Sa_Ts_deg_k[indx],ln_d_k[indx],d_k[indx])
			
			row_check = d[0].row
			col_check = d[0].col
			data_check = d[0].data
			print(row_check[:2],col_check[:2],data_check[:2])
