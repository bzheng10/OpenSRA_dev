...
class GMPE(object):
    ...
    def run_check(cls, rtol=0, atol=1e-3):
        ...
        test_file_dir = os.path.join('test','im','gmpe')
        ...
        for i in range(test_inputs.shape[0]):
            ...
            # set inputs
            instance.set_all_inputs(kwargs)
            instance.run_model(njit_on=False)
            # check relative error on mean
            assert_allclose(
                instance.model_dist['mean'][:,0], # calculation
                np.log(test_median.loc[i].values), # true
                rtol=rtol, atol=atol, equal_nan=True,
            )
            # check relative error on sigma
            assert_allclose(
                instance.model_dist['aleatory']['sigma'][:,0], # calculation
                test_sigma.loc[i].values, # true
                rtol=rtol, atol=atol, equal_nan=True,
            )
        # print message if able to check through all cases
        print(f"Tests pass for {gmpe_name}")
