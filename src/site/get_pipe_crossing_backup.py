    ...
    if not im_source == 'UCERF':
        ...
        for each in['primary','secondary']:
            ...
            slip_vect_dx_dy_norm = np.asarray(
                get_dx_dy_for_vector_along_azimuth(azimuth)
            ).T
            ...
            # ------------------------------------
            # this section is used to get fault disp metrics
            ...
            prob_surf_rup_list = [] # tracking probability of surface fault rupture
            # fault_disp_list = [] # get displacement
            prob_crossing_list = [] # probability of crossing
            # prob_crossing_sf_list = [] # probability of crossing scale factor for secondary hazard
            # prob_disp_sf_list = [] # displacement scale factor for secondary hazard
            ...
            if not fault_disp_model == 'PetersenEtal2011':
                ...
                for i in range(len(segment_crossed)):
                    scenario_crossed_list.append([])
                    norm_dist_list.append([])
                    prob_surf_rup_list.append([])
                    prob_crossing_sf_list.append([])
                    prob_crossing_list.append([])
                    prob_disp_sf_list.append([])
                ...
                # get crossing angles
                segment_crossed_azimuth = get_azimuth(
                    segment_crossed_start,segment_crossed_end
                )
                crossing_theta = azimuth_lines(
                    segment_crossed_azimuth, # segment azimuth
                    strike, # qfault strike = dipdir - 90
                )
                # probability of crossing
                norm_seg_len = segment_by_qfault[each].LENGTH_KM.values*1000/200
                prob_crossing = np.round(
                    norm_seg_len*np.abs(np.sin(np.radians(crossing_theta)))
                ,decimals=4)
                ...
                for i in range(len(segment_crossed)):
                    ...
                    gdf_norm_scenario_utm_crossed_i = \
                        rupture_table_utm.iloc[rows_for_parent_id_crossed_i].copy()
                    # scenario considered as crossed for current segment
                    scenario_crossed_seg_i = []
                    # tracking normalized distance from endpoint to fault
                    norm_dist_seg_i = []
                    # tracking probability of surface fault rupture
                    prob_surf_rup_seg_i = []
                    # for each scenario
                    for j in range(gdf_norm_scenario_utm_crossed_i.shape[0]):
                        ...
                        # round to avoid roundoff errors getting intersections
                        ...
                        if (nearest_pt_j[0] == fault_trace_j_arr[0][0] and \
                            ...
                            # if False, consider scenario as crossed for segment
                            ...
                            if not fault_ind_j in segment_crossed_by_rupture[each]:
                    ...
                    norm_dist_list.append(list(np.round(norm_dist_seg_i,decimals=3)))
                    # get probability of surface fault rupture and displacement
                    # if len(scenario_crossed_seg_i) > 0:
                    #     # prob surf rup
                    #     # mags = gdf_norm_scenario_utm_crossed_i.Magnitude.loc[scenario_crossed_seg_i].values
                    #     # prob_surf_rup_list.append(list(np.round(
                    #     #     np.exp(-12.51+2.053*mags)/(1+np.exp(-12.51+2.053*mags))
                    #     # ,decimals=4)))
                    #     # scale factor for probablity of crossing
                    #     # if 'primary' in each:
                    #     #     prob_crossing_sf_list.append(list([1]*len(scenario_crossed_seg_i))) # no scale factor applied
                    #     #     prob_disp_sf_list.append(list([1]*len(scenario_crossed_seg_i))) # no scale factor applied
                    #     # else:
                    #     #     #########################################
                    #     #     # placeholder for secondary scale factors
                    #     #     # for now use a constant until Steve Thompson provides triangular distribution.
                    #     #     # --- for crossing
                    #     #     prob_crossing_sf_list.append(list([0.1]*len(scenario_crossed_seg_i))) # no scale factor applied
                    #     #     # apply triangular distribution, with peak at 0.5 and truncated at 0.2 and 0.8
                    #     #     # prob_surf_rup_curr = np.asarray(prob_surf_rup_list[-1])
                    #     #     # prob_crossing_sf_curr = prob_surf_rup_curr
                            
                    #     #     # --- for displacement
                    #     #     prob_disp_sf_list.append(list([0.1]*len(scenario_crossed_seg_i))) # no scale factor applied
                    #     #     #########################################
                    #     # get probability of crossing with scale factors applied
                    #     prob_crossing_list.append(list(
                    #         prob_crossing[i]*np.asarray(prob_crossing_sf_list[-1])
                    #     ))
                    #     # disp magnitude by Petersen et al. (2011)
                    #     # x_star = np.sqrt(1 - ((np.asarray(norm_dist_list[-1])-0.5)/0.5)**2)
                    #     # fault_disp_list_curr = np.exp(1.7927*mags + 3.3041*x_star - 11.2192) / 100 # in meters
                    #     # scale displacements by scale factor, ln_d * SF
                    #     # fault_disp_list_curr = fault_disp_list_curr * \
                    #     #     np.exp(np.asarray(prob_disp_sf_list[-1]))
                    #     # append to list
                    #     # fault_disp_list.append(list(np.round(fault_disp_list_curr,decimals=4)))
                    #     # also append to dictionary that tracks displacement metrics
                    #     for k in range(len(scenario_crossed_seg_i)):
                    #         if not scenario_crossed_seg_i[k] in prob_surf_rup_by_rupture[each]:
                    #             prob_surf_rup_by_rupture[each][scenario_crossed_seg_i[k]] = [prob_surf_rup_list[-1][k]]
                    #             prob_crossing_by_rupture[each][scenario_crossed_seg_i[k]] = [prob_crossing_list[-1][k]]
                    #             norm_dist_by_rupture[each][scenario_crossed_seg_i[k]] = [norm_dist_list[-1][k]]
                    #             # pgdef_m_by_rupture[each][scenario_crossed_seg_i[k]] = [fault_disp_list[-1][k]]
                    #         else:
                    #             prob_surf_rup_by_rupture[each][scenario_crossed_seg_i[k]].append(prob_surf_rup_list[-1][k])
                    #             prob_crossing_by_rupture[each][scenario_crossed_seg_i[k]].append(prob_crossing_list[-1][k])
                    #             norm_dist_by_rupture[each][scenario_crossed_seg_i[k]].append(norm_dist_list[-1][k])
                    #             # pgdef_m_by_rupture[each][scenario_crossed_seg_i[k]].append(fault_disp_list[-1][k])
                    # else:
                    #     # append empty list
                    #     prob_surf_rup_list.append([])
                    #     prob_crossing_sf_list.append([])
                    #     prob_crossing_list.append([])
                    #     # prob_disp_sf_list.append([])
                    #     # fault_disp_list.append([])
                    
            # append crossing logic results to segment crossed dataframe
            # segment_by_qfault[each][f'qfault_crossed_{each}'] = qfault_crossed
            # segment_by_qfault[each][f'psi_dip_{each}'] = np.round(dip,decimals=1)
            # segment_by_qfault[each][f'theta_rake_{each}'] = np.round(rake,decimals=1)
            # segment_by_qfault[each][f'rake_azimuth_{each}'] = np.round(azimuth,decimals=1)
            # segment_by_qfault[each][f'crossing_coord_{each}'] = crossing_coords.tolist()
            # segment_by_qfault[each][f'beta_crossing_{each}'] = beta_crossing
            # segment_by_qfault[each][f'scenario_crossed_{each}'] = scenario_crossed_list
            # segment_by_qfault[each][f'norm_dist_{each}'] = norm_dist_list
            # segment_by_qfault[each][f'prob_surf_rup_{each}'] = prob_surf_rup_list
            # # segment_by_qfault[each][f'pgdef_m_{each}'] = fault_disp_list
            # segment_by_qfault[each][f'prob_crossing_{each}'] = prob_crossing_list
            # segment_by_qfault[each][f'prob_crossing_sf_{each}'] = prob_crossing_sf_list
            # segment_by_qfault[each][f'prob_disp_sf_{each}'] = prob_disp_sf_list
            ...
            segment_by_qfault[each][f'crossing_coord'] = crossing_coords.tolist()
            segment_by_qfault[each][f'beta_crossing'] = beta_crossing
            segment_by_qfault[each][f'scenario_crossed'] = scenario_crossed_list
            segment_by_qfault[each][f'norm_dist'] = norm_dist_list
            segment_by_qfault[each][f'prob_surf_rup'] = prob_surf_rup_list
            # segment_by_qfault[each][f'pgdef_m_{each}'] = fault_disp_list
            segment_by_qfault[each][f'prob_crossing'] = prob_crossing_list
            segment_by_qfault[each][f'prob_crossing_sf'] = prob_crossing_sf_list
            # segment_by_qfault[each][f'prob_disp_sf'] = prob_disp_sf_list
            ...
            segment_by_qfault[each][f'l_anchor'] = anchorage_length
        ...
        for each in ['primary','secondary']:
            ...
            for i,scen in enumerate(segment_by_qfault[each][f'scenario_crossed'].values):
                if len(scen)>0:
        ...
        scenarios_with_crossing_for_haz = np.unique(np.hstack([
            ...
            np.hstack(segment_by_qfault[each][f'scenario_crossed'].values)
            for each in ['primary','secondary']
        ])).astype(int)
        # sort lists under segment_crossed_by_rupture
        for each in segment_crossed_by_rupture:
        ...
        for each in ['primary','secondary']:
            ...
            # rupture_table_crossing_only[f'pgdef_m_crossed_{each}'] = pgdef_m_curr_haz
            # get columns created from this function
            col_headers_to_append.append([
                f'seg_ind_crossed_{each}',
                f'prob_surf_rup_for_seg_{each}',
                f'prob_crossing_for_seg_{each}',
                f'norm_dist_for_seg_{each}',
            ])
        col_headers_to_append = np.asarray(col_headers_to_append).flatten()
        # rupture_table_crossing_only['sigma'] = 0.498
        # rupture_table_crossing_only['sigma_mu'] = 1.0197
        # event_ids_to_keep = rupture_table_crossing_only.EventID.values
        ...
        # if all_segments_crossed_gdf.shape[0] > 0:
        #     all_segments_crossed_gdf.to_file(
        #         os.path.join(processed_input_dir,f'site_data_PROCESSED_CROSSING_ONLY_GEOMS.shp')
        #     )
            
        # ---
        # return
        # return all_segments_crossed, rupture_table_crossing_only, col_headers_to_append, segment_ids_to_keep
        # return all_segments_crossed, event_ids_to_keep
        return all_segments_crossed, rupture_table_crossing_only, col_headers_to_append
