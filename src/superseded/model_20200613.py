...
class assessment(object):
	...
	def create_RV(self, im_tool, site_loc_file, im_dir, im_list=['pga,pgv'], rup_meta_file=None, 
					vs30_file=None, flag_clear_dict=False, flag_sample_exist=False, **kwargs):
		"""
		...
		# print("read site locations and rupture information--- %10.6f seconds ---" % (time.time() - start_time))
		# start_time = time.time()
		...
		##
			...
			# im_in = file_io.read_sha_sparse(im_dir, group_num, num_rups, im_list)
			self._RV_dict.update({'rup':file_io.read_sha_sparse(im_dir, rup_group, 
	...
	#############################################################################################################
	def get_IM(self, nsamp, im_list=['pga,pgv'], flag_corr_d=True, flag_corr_T=True, T_pga=0.01, T_pgv=1.0, 
					method_d='jayaram_baker_2009', method_T='baker_jayaram_2008', 
                    flag_clear_dict=False, flag_sample_exist=False, path_sample=None, **kwargs):
		"""
		...
		# rate = self._RV_dict['rup'][key].get('rate',None)
		# sites = self._RV_dict['rup'][key].get('sites',None)
		# dim_rup = len(self._RV_dict['rup'][key].get('src_rup',None))
		...
		# if sites is None:
			## for EventCalc
			# pga_intra = np.ones*self._RV_dict.get('pga_sigma',None)['sig_intra']
			# pga_inter = np.ones*self._RV_dict.get('pga_sigma',None)['sig_inter']
			# pgv_intra = np.ones*self._RV_dict.get('pgv_sigma',None)['sig_intra']
			# pgv_inter = np.ones*self._RV_dict.get('pgv_sigma',None)['sig_inter']
			
		# else:
			## for OpenSHAInterface
			# dim_d = len(sites) # number of sites
		# dim_d = len(self._rup_site_dict.get('site_lon',None))
		# start_time = time.time()
		...
		if flag_corr_d is False and flag_corr_T is False:
			...
			if flag_corr_d is True:
				...
				# print("get triu indices --- %10.6f seconds ---" % (time.time() - start_time))
			...
			# print("create covariance matrices for intra-event sigma --- %10.6f seconds ---" % (time.time() - start_time))
			...
			# print("create covariance matrices for inter-event sigma --- %10.6f seconds ---" % (time.time() - start_time))
	...
	def get_EDP(self, category, method, return_param, store_name=None, flag_clear_dict=False, 
		...
		## dimensions
		#try:
		#	shape = self._IM_dict['pga'][0].shape
		#except:
		#	try:
		#		shape = self._IM_dict['pgv'][0].shape
		#	except:
		#		shape = None
		...
		#	dim_d = shape[1]
		#	# dim_T = dim_T = len(im_list) # number of periods of interest
		#	dim_rup = shape[0]
		#else:
		#	dim_d = None
		#	dim_rup = None
		...
		## pass sites
		# if flag_site is True:
			# kwargs['site_lon'] = self._rup_site_dict.get('site_lon',None)
			# kwargs['site_lat'] = self._rup_site_dict.get('site_lat',None)
		...
		## if either pga or pgv is required, run procedure through all scenarios
			...
			if flag_pga is True:
				# kwargs['pga'] = np.transpose(self._IM_dict['pga'])
				kwargs['pga'] = self._IM_dict['pga']
			if flag_pgv is True:
				# kwargs['pgv'] = np.transpose(self._IM_dict['pgv'])
			...
			#	## get IM simulations for scenario
			#	if flag_pga is True:
			#		kwargs['pga'] = np.transpose(self._IM_dict[i]['pga'])
			#	if flag_pgv is True:
		...
		for i in return_param:
			...
			# self._EDP_dict[i][name].update({'output': output[i].copy()})
			# store_dict = {}
	...
	def get_DM(self, category, method, return_param, store_name=None, im_list=['pga,pgv'], flag_clear_dict=False, 
		...
		# if 'event' in im_tool.lower() or 'calc' in im_tool.lower():
			# dim_d = len(self._RV_dict['rup'][0]['pga_mean'])
			# dim_rup = len(self._RV_dict['rup'])
		# dim_d = dim_d = len(self._rup_site_dict.get('site_lon',None))
		dim_T = len(im_list) # number of periods of interest
		# dim_rup = len(self._RV_dict['rup'])
		# dim_rup = 100
		...
		if flag_pga is False and flag_pgv is False and flag_rup_depend is False:
			...
			for i in return_param:
				...
				if count == 0:
					...
					for key in self._DM_dict[i].keys():
						if self._DM_dict[i][key]['method'] == method and \
							...
							self._DM_dict[i][key]['source_method'] == source_method:
				...
				# result_updated = np.sum(output[i]['all_rup'].multiply(rate).toarray(),axis=0)
				result_updated = sparse.csc_matrix(np.sum(output[i]['all'].multiply(rate).toarray(),axis=0))
				
				# print('0: return_param = '+i+', method = '+method+', name = '+name)
		...
		## if either pga or pgv is required, run procedure through all scenarios
			...
			for i in return_param:
				...
				########################################
				#######################
				###################### need to figure out how to store new methods versus continue old methods
				# self._DM_dict[i].update({name: {'method':method,
												# 'source_param':source_param,
												# 'source_method':source_method}})
												
				# for j in range(nsamp):
					# result_curr = output[i][j].multiply(rate)
				
					# print(self._DM_dict)
				
					# if 'output' in self._DM_dict[i][name].keys():
						# result_updated = self._DM_dict[i][name]['output'][j] + result_curr
						# self._DM_dict[i][name]['output'].update({j:result_updated})
					# else:
						# self._DM_dict[i][name].update({'output':{j:result_curr}})
				...
				#############################
				######### for Marc
				# out_samp = output[i][5].toarray()
				# ind_marc = [41305, 41312, 29515, 40960, 38581, 76484, 530, 523]
				# rup_marc = [10, 8, 62]
				# for rup in rup_marc:
					# print(rup)
					# print(method)
					# print(out_samp[rup,ind_marc])
					# print(' ')
				####################################
				
				if 'all' in output[i].keys():
				# if nsamp == 1:
					# result_curr = output[i]['all'].multiply(rate).tocsc()
					result_curr = sparse.csc_matrix(np.sum(output[i]['all'].multiply(rate).toarray(),axis=0))
				else:
					# result_curr = np.asarray([np.sum(output[i][j].multiply(rate).toarray(),axis=0) for j in range(nsamp)])
					result_curr = sparse.csc_matrix([np.sum(output[i][j].multiply(rate).toarray(),axis=0) for j in range(nsamp)])
				
				try:
					result_updated = self._DM_dict[i][name]['output'] + result_curr
					# print('1: return_param = '+i+', method = '+method+', name = '+name)
				except:
					result_updated = result_curr
					# print('2: return_param = '+i+', method = '+method+', name = '+name)
			
				self._DM_dict[i][name].update({'output':result_updated})
				# print(self._DM_dict)
				
				# store_dict = {}
				# store_dict.update({key2: output[key2] for key2 in output[i].keys()})
				# self._DM_dict[key1][i][name].update({'output': store_dict.copy()})
				# self._DM_dict[key1][i][name].update({'output': output[i].copy()})
